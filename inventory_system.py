...
def parse_csv_row(row: str) -> Dict:
    ...
    price = fields[2]
    quantity = fields[3]
    category = (fields[5] if len(fields) > 5 else None)  # ERROR 11: IndexError — only 5 fields (0-4), index 5 is OOB
...
def validate_shipment(manifest_str: str) -> Dict:
    ...
    origin = parts[0]
    destination = parts[1]
    weight = (parts[3] if len(parts) > 3 else None)  # ERROR 19: IndexError — only 3 parts (0,1,2), index 3 is OOB
